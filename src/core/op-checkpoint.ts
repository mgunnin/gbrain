...
export async function recordCompleted(
  ...
  // exactly as before. Pass a native text[] and convert with `to_jsonb($3::text[])`:
  // postgres.js + PGLite both handle string[] as text[], and this avoids the
  // double-encode trap where a JSON-string parameter becomes a JSONB scalar and
  // violates op_checkpoints_completed_keys_array.
